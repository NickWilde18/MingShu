...
func ExistUPN(ctx context.Context, upn string) {
	...
	if response.IsNil() || response.IsEmpty() {
		m.RenderError(g.RequestFromCtx(ctx), m.ErrorInfo{
			...
			Detail: "校验用户信息：内部请求没有响应或返回内容为空。<br/>The internal request does not respond or the returned content is empty.",
		})
	}
	...
	if !content["success"].(bool) {
		m.RenderError(g.RequestFromCtx(ctx), m.ErrorInfo{
			...
			Detail: `无法查找到你的AD域信息。这通常不是你的问题。<br/>
如果您是新入职/入学的人员，请在1天后再登录。<br/>
如您已经入职/入学1天以上，或想立即进入平台，请联系管理员手动同步AD域数据库。<br/>
<br/>
AD domain information could not be found. This is usually not your issue.<br/>
If you are a new employee/student, please try logging in again after 1 day.<br/>
