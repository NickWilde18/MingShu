...
func EnsureQP(ctx context.Context, upn string) {
	...
	if response.IsNil() || response.IsEmpty() {
		m.RenderError(g.RequestFromCtx(ctx), m.ErrorInfo{
			...
			Detail:    "个人配额池初始化：内部请求没有响应或返回内容为空。<br/>The personal quota pool initialization: the internal request does not respond or the returned content is empty.",
		})
	}
	...
	if !content["success"].(bool) {
		m.RenderError(g.RequestFromCtx(ctx), m.ErrorInfo{
			ErrorCode: consts.ErrCodeBadGateway,
			Detail:    "个人配额池存在性检查失败：" + content["message"].(string) + "<br/>The personal quota pool existence check failed: " + content["message"].(string),
		})
	}
}
